...
export class MessagesController {
  ...
  @ApiResponse({ status: 200, description: 'List of conversations sorted by most recent message' })
  ...
  @Get(':id/messages')
  @ApiOperation({ summary: 'Get messages in a conversation (marks unread as read)' })
  @ApiParam({ name: 'id', description: 'Conversation UUID' })
  @ApiResponse({ status: 200, description: 'List of messages in chronological order' })
  getMessages(
  ...
  @ApiResponse({ status: 201, description: 'Returns { conversation, message }' })
  startConversation(
    @Body() dto: StartConversationDto,
    @CurrentUser('id') userId: string,
  ) {
    return this.messagesService.startConversation(userId, dto.listingId, dto.body);
  }

  @Post(':id/messages')
  @ApiOperation({ summary: 'Send a message in a conversation' })
  @ApiParam({ name: 'id', description: 'Conversation UUID' })
  @ApiResponse({ status: 201, description: 'The created message with sender info' })
  sendMessage(
    @Param('id', ParseUUIDPipe) id: string,
